...
def process_num_centers(img):
    centers, _ = detect_scattering_centers(img)
    return len(centers)


def process_centers_positions(img, max_centers):
    centers, _ = detect_scattering_centers(img)
...
def process_centers_amplitudes(img, max_centers):
    _, amplitudes = detect_scattering_centers(img)
    amps = np.array(amplitudes[:max_centers])
    pad_len = max_centers - min(len(amplitudes), max_centers)
    if pad_len > 0:
        amps = np.pad(amps, (0, pad_len), 'constant')
    return amps


def process_scattering_centroid(img):
...
def process_max_amplitude(img):
    _, amplitudes = detect_scattering_centers(img)
    return np.max(amplitudes) if len(amplitudes) > 0 else 0.0


def process_min_amplitude(img):
    _, amplitudes = detect_scattering_centers(img)
    return np.min(amplitudes) if len(amplitudes) > 0 else 0.0
...
def num_centers(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_num_centers)(im) for im in images
    ))


def centers_positions(images, max_centers=32, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_centers_positions)(im, max_centers) for im in images
    ))


def centers_amplitudes(images, max_centers=32, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_centers_amplitudes)(im, max_centers) for im in images
    ))


def scattering_centroid(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_scattering_centroid)(im) for im in images
    ))
...
def max_amplitude(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_max_amplitude)(im) for im in images
    ), dtype=np.float32)


def min_amplitude(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_min_amplitude)(im) for im in images
    ), dtype=np.float32)
...
def deep_scattering_features(images, out_channels=8, batch_size=4):
