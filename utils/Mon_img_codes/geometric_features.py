...
def process_area(img, num=10):
    ...
    for i in range(num):
        if i >= len(contours):
            break
        features[i] = cv2.contourArea(contours[i])
    return features


def process_perimeter(img, num=10):
    features = np.zeros(num)
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
        if i >= len(contours):
            break
        features[i] = cv2.arcLength(contours[i], True)
    return features


def process_aspect_ratio(img, num=10):
    features = np.zeros(num)
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
        ...
        features[i] = float(w) / h if h != 0 else 0
    return features


def process_rectangularity(img, num=10):
    features = np.zeros(num)
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
        ...
        x, y, w, h = cv2.boundingRect(contours[i])
        rect_area = w * h
        features[i] = area / rect_area if rect_area != 0 else 0
    return features


def process_sphericity(img, num=10):
    features = np.zeros(num)
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
        ...
        features[i] = (4 * np.pi * area) / (perimeter ** 2) if perimeter != 0 else 0
    return features


def process_hu_moments(img, num=10):
    features = []
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
...
def process_canny(img, down_size=(16, 16), threshold1=100, threshold2=200):
...
def process_sobel(img, down_size=(8, 8), ksize=3):
    ...
    sobel_feature = np.hstack([sobel_x.flatten(), sobel_y.flatten()])
    return sobel_feature


def process_log(img, down_size=(16, 16), ksize=5, sigma=1.0):
    gray = img.astype(np.uint8) if len(img.shape) == 2 else cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
def extract_area(images, num=10, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_area)(im, num) for im in images))


def extract_perimeter(images, num=10, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_perimeter)(im, num) for im in images))


def extract_aspect_ratio(images, num=10, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_aspect_ratio)(im, num) for im in images))


def extract_rectangularity(images, num=10, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_rectangularity)(im, num) for im in images))


def extract_sphericity(images, num=10, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_sphericity)(im, num) for im in images))


def extract_hu_moments(images, num=10, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_hu_moments)(im, num) for im in images))


def extract_canny_features(images, down_size=(16, 16), threshold1=100, threshold2=200, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_canny)(im, down_size, threshold1, threshold2) for im in images))


def extract_sobel_features(images, down_size=(8, 8), ksize=3, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_sobel)(im, down_size, ksize) for im in images))


def extract_log_features(images, down_size=(16, 16), ksize=5, sigma=1.0, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(process_log)(im, down_size, ksize, sigma) for im in images))
