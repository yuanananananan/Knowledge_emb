...
# 内部工具函数
# ======================
def _glcm_stat(img, distances, angles, levels, prop):
    return graycoprops(
        graycomatrix(img.astype(np.uint8),
                     distances=distances,
                     angles=angles,
                     levels=levels,
                     symmetric=True,
...
def _gabor_mean(img, frequency, theta):
    return np.sqrt(gabor(img, frequency=frequency, theta=theta)[0]**2 +
                   gabor(img, frequency=frequency, theta=theta)[1]**2).mean(axis=1)


def _gabor_variance(img, frequency, theta):
    return np.sqrt(gabor(img, frequency=frequency, theta=theta)[0]**2 +
                   gabor(img, frequency=frequency, theta=theta)[1]**2).var(axis=1)


def _gabor_energy(img, frequency, theta):
    return np.mean(
        np.sqrt(gabor(img, frequency=frequency, theta=theta)[0]**2 +
                gabor(img, frequency=frequency, theta=theta)[1]**2)**2,
        axis=1
    )
...
def glcm_contrast(images, distances=[1], angles=[0], levels=256, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_glcm_stat)(im, distances, angles, levels, 'contrast')
        for im in images
    ))
...
def glcm_dissimilarity(images, distances=[1], angles=[0], levels=256, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_glcm_stat)(im, distances, angles, levels, 'dissimilarity')
        for im in images
    ))
...
def glcm_homogeneity(images, distances=[1], angles=[0], levels=256, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_glcm_stat)(im, distances, angles, levels, 'homogeneity')
        for im in images
    ))
...
def glcm_correlation(images, distances=[1], angles=[0], levels=256, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_glcm_stat)(im, distances, angles, levels, 'correlation')
        for im in images
    ))
...
def glcm_energy(images, distances=[1], angles=[0], levels=256, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_glcm_stat)(im, distances, angles, levels, 'energy')
        for im in images
    ))
...
def lbp_histogram(images, P=4, R=1, method='default', normalize=True, n_jobs=-1):
    ...
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_lbp_hist)(im, P, R, method, n_bins, normalize)
        for im in images
    ))
...
def gabor_mean(images, frequency=0.2, theta=0, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_gabor_mean)(im, frequency, theta)
        for im in images
    ))
...
def gabor_variance(images, frequency=0.2, theta=0, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_gabor_variance)(im, frequency, theta)
        for im in images
    ))
...
def gabor_energy(images, frequency=0.2, theta=0, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(_gabor_energy)(im, frequency, theta)
        for im in images
    ))
