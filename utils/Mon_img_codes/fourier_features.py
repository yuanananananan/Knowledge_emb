...
def fft_mean(imgs):
    mags, phases = fft2_image(imgs)
    mean = np.mean(mags, axis=(2))
    return mean

def fft_var(imgs):
    mags, phases = fft2_image(imgs)
    var = np.var(mags, axis=(2))
    return var
...
def fft_directional_energy_ratio(imgs, angle_bins=4):
    mags, phases = fft2_image(imgs)
    N, h, w = mags.shape
    cy, cx = h // 2, w // 2
    Y, X = np.indices((h, w))
...
def deep_fourier_features(images, out_channels=64, batch_size=4):
