...
def _process_gray_histogram(img, bins=64):
    gray = img.astype(np.uint8)
    hist = cv2.calcHist([gray], [0], None, [bins], [0, 256])
    return _normalize_hist(hist)


def _process_gray_moments(img):
    gray = img.astype(np.uint8)
    ...
    mean = np.mean(pixels)
    std = np.std(pixels)
    skewness = skew(pixels) if std > 0 else 0.0
    return np.array([mean, std, skewness], dtype=np.float32)


def _process_gray_ccv(img, bins=16, threshold=50):
    gray = img.astype(np.uint8)
    quantized = (gray // (256 // bins)).astype(np.uint8)
    total_pixels = gray.size if gray.size > 0 else 1
    ...
    return np.array(ccv, dtype=np.float32)


def _process_gray_blocks_histogram(img, grid=(4, 4), bins=8):
    gray = img.astype(np.uint8)
    h, w = gray.shape
    ...
    for i in range(grid[0]):
        for j in range(grid[1]):
            ...
            feats.extend(_normalize_hist(hist))
    return np.array(feats, dtype=np.float32)


def _process_gray_autocorrelogram(img, distance=1, bins=16):
    gray = img.astype(np.uint8)
    quantized = (gray // (256 // bins)).astype(np.uint8)
    h, w = quantized.shape
            ...
            for dx, dy in offsets:
                ...
                if 0 <= nx < w and 0 <= ny < h:
                    ...
                    if quantized[ny, nx] == gray_val:
                        match += 1
        correlogram[gray_val] = match / count if count > 0 else 0.0
    return correlogram
...
def gray_histogram(images, bins=64, n_jobs=-1):
    return np.array(
        Parallel(n_jobs=n_jobs)(
            delayed(_process_gray_histogram)(im, bins) for im in images
        )
    )
...
def gray_moments(images, n_jobs=-1):
    return np.array(
        Parallel(n_jobs=n_jobs)(
            delayed(_process_gray_moments)(im) for im in images
        )
    )
...
def gray_ccv(images, bins=16, threshold=50, n_jobs=-1):
    return np.array(
        Parallel(n_jobs=n_jobs)(
            delayed(_process_gray_ccv)(im, bins, threshold) for im in images
        )
    )
...
def gray_blocks_histogram(images, grid=(4, 4), bins=8, n_jobs=-1):
    return np.array(
        Parallel(n_jobs=n_jobs)(
            delayed(_process_gray_blocks_histogram)(im, grid, bins) for im in images
        )
    )
...
def gray_autocorrelogram(images, distance=1, bins=16, n_jobs=-1):
    return np.array(
        Parallel(n_jobs=n_jobs)(
            delayed(_process_gray_autocorrelogram)(im, distance, bins) for im in images
        )
    )
