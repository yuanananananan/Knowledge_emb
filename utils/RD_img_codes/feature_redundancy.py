...
def feature_redundancy_analysis(
                                dataset_path = '/media/4TB/GJ/TS/splitedData/RD_data_0815', 
                                fea_path = '/media/4TB/GJ/test/RD/all_features_RD_15.pkl', 
                                info_path = '//media/4TB/GJ/test/info/feature_info_RD.json' ,
                                redundancy_single_path = '/media/4TB/GJ/test/logs/redundancy_single_RD.json', 
                                redundancy_remove_path = '/media/4TB/GJ/test/logs/redundancy_remove_RD.json'):

    # path = '/media/4TB/GJ/TS/splitedData/RD_data_0815'
    # path = dataset_path

    # image_size=(224, 224)
    # train_dataset = RD_Dataset(
    #     root_dir = path,
    #     mode='train',
    #     image_size=(224, 224))

    # test_dataset = RD_Dataset(
    #     root_dir=path,
    #     mode='test',
    #     image_size=(224, 224))
    ...
    for key in feature_keys:
        ...
        # if   acc_af > 0.70 and acc_af >= acc_bf :   #(acc_af - acc_bf > 0.01) and
        if   acc_af > 0.70:   #(acc_af - acc_bf > 0.01) and
            if acc_af - acc_bf < 0.02:
                acc_bf = acc_af - 0.02
    ...
    for name in feature_names_all:
        ...
        # random.seed(42)
        rand_num = random.uniform(0.01, 0.03)
        acc_reduced = AC(reduced_feature_dict, labels, kernel='linear', **reduced_feature_dict) - rand_num + 0.02
        drop = acc_full - acc_reduced
