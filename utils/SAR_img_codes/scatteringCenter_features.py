...
# ---------------- 单张图像处理函数 ----------------
def num_centers_single(img):
    return len(detect_scattering_centers(img)[0])

def centers_positions_single(img, max_centers=32):
...
def centers_amplitudes_single(img, max_centers=32):
    amplitudes = np.array(detect_scattering_centers(img)[1][:max_centers])
    pad_len = max_centers - len(amplitudes)
    if pad_len > 0:
        amplitudes = np.pad(amplitudes, (0, pad_len), 'constant')
    return amplitudes

def scattering_centroid_single(img):
...
def max_amplitude_single(img):
    amps = detect_scattering_centers(img)[1]
    return np.max(amps) if len(amps) > 0 else 0.0

def min_amplitude_single(img):
    amps = detect_scattering_centers(img)[1]
    return np.min(amps) if len(amps) > 0 else 0.0

# ---------------- 并行处理接口 ----------------
def num_centers(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(num_centers_single)(im) for im in images))

def centers_positions(images, max_centers=32, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(centers_positions_single)(im, max_centers) for im in images))

def centers_amplitudes(images, max_centers=32, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(centers_amplitudes_single)(im, max_centers) for im in images))

def scattering_centroid(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(scattering_centroid_single)(im) for im in images))

def scattering_std(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(scattering_std_single)(im) for im in images))

def max_amplitude(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(max_amplitude_single)(im) for im in images), dtype=np.float32)

def min_amplitude(images, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(delayed(min_amplitude_single)(im) for im in images), dtype=np.float32)
...
def deep_scattering_features(images, out_channels=64, batch_size=4):
