...
# 1. 极差
def get_range(image):
    return np.max(image, axis=(1, 2)) - np.min(image, axis=(1, 2))
...
# 4. 四分位差
def get_iqr(image):
    q3 = np.percentile(image, 75, axis=(1, 2))
    q1 = np.percentile(image, 25, axis=(1, 2))
    return q3 - q1

# 5. 平均绝对偏差
def get_mad(image):
    mean_val = np.mean(image, axis=(1, 2), keepdims=True)
    return np.mean(np.abs(image - mean_val), axis=(1, 2))

# 6. 三阶中心矩
def get_third_central_moment(image):
    mean_val = np.mean(image, axis=(1, 2), keepdims=True)
    return np.mean((image - mean_val)**3, axis=(1, 2))

# 7. 四阶中心矩
def get_fourth_central_moment(image):
    mean_val = np.mean(image, axis=(1, 2), keepdims=True)
    return np.mean((image - mean_val)**4, axis=(1, 2))

# 8. 变异系数
def get_cv(image):
    mean_val = np.mean(image, axis=(1, 2))
    std_val = np.std(image, axis=(1, 2))
...
class ReliefSeqNet(nn.Module):
    ...
    使用 LSTM 提取图像起伏特征
    """
    def __init__(self, hidden_dim=128, output_dim=64, num_layers=1, bidirectional=True):
        super().__init__()
    ...
    def _extract_mini_batch(self, images):
        ...
        for img in images:
            if img.ndim == 3:
                ...
                gray = img.astype(np.float32).astype(np.float32) / 255.0
...
# ----------------- 使用示例 -----------------
def deep_relief_seq_features(images, output_dim=64, batch_size=4, hidden_dim=64):
