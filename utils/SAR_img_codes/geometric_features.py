...
def process_aspect_ratio(img, num):
    features = np.zeros(num)
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
...
def process_rectangularity(img, num):
    features = np.zeros(num)
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
...
def process_sphericity(img, num):
    features = np.zeros(num)
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
...
def process_hu_moments(img, num):
    features = []
    _, binary_image = cv2.threshold(img.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(num):
...
def process_canny(img, down_size, threshold1, threshold2):
...
def process_sobel(img, down_size, ksize):
...
def process_log(img, down_size, ksize, sigma):
    gray = img.astype(np.uint8) if len(img.shape) == 2 else cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
# 特征提取接口
# ======================
def extract_area(images, num=1, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_area)(im, num) for im in images))

def extract_perimeter(images, num=1, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_perimeter)(im, num) for im in images))

def extract_aspect_ratio(images, num=1, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_aspect_ratio)(im, num) for im in images))

def extract_rectangularity(images, num=1, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_rectangularity)(im, num) for im in images))

def extract_sphericity(images, num=1, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_sphericity)(im, num) for im in images))

def extract_hu_moments(images, num=1, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_hu_moments)(im, num) for im in images))

def extract_canny_features(images, down_size=(8,8), threshold1=100, threshold2=200, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_canny)(im, down_size, threshold1, threshold2) for im in images))

def extract_sobel_features(images, down_size=(8,8), ksize=3, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_sobel)(im, down_size, ksize) for im in images))

def extract_log_features(images, down_size=(8,8), ksize=5, sigma=1.0, n_jobs=-1):
    return np.array(Parallel(n_jobs=n_jobs)(
        delayed(process_log)(im, down_size, ksize, sigma) for im in images))
