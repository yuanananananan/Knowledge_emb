...
class RD_Dataset(Dataset):
            ...
            if os.path.isdir(class_dir):
                ...
                for root, _, files in os.walk(class_dir):
                    for img_name in files:
                        ...
                        if img_path.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.npy')):  # 确保是图像文件
    ...
    def __getitem__(self, idx):
        ...
        data_imag = data.imag
        img_source = data_abs
        img_source = img_source / (img_source.max() + 1e-6)
        img_source = np.log1p(img_source) * 255
...
def get_rd_dataset(cfg):
    ...
    dataset = RD_Dataset(root_dir=root_dir, transform=transform, cfg=cfg)
